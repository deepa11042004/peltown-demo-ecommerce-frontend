...
const proxyAddressesRequest = async (request: NextRequest, context: RouteContext) => {
  ...
  const pathnameSuffix = path && path.length > 0
    ...
    : "";
  const targetUrl = `${apiBaseUrl}/api/v1/addresses${pathnameSuffix}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: "Unable to reach address service",
...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyAddressesRequest(request, context);
}
