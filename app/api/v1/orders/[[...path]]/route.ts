...
const proxyOrdersRequest = async (request: NextRequest, context: RouteContext) => {
  ...
  const pathnameSuffix = path && path.length > 0
    ...
    : "";
  const targetUrl = `${apiBaseUrl}/api/v1/orders${pathnameSuffix}${request.nextUrl.search}`;
...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyOrdersRequest(request, context);
}
