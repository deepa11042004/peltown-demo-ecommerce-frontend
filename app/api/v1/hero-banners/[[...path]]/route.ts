...
const proxyHeroBannerRequest = async (request: NextRequest, context: RouteContext) => {
  ...
  const pathnameSuffix = path && path.length > 0
    ...
    : "";
  const targetUrl = `${apiBaseUrl}/api/v1/hero-banners${pathnameSuffix}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: "Unable to reach hero banner service",
...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyHeroBannerRequest(request, context);
}
