...
const proxyAdminOrdersRequest = async (request: NextRequest, context: RouteContext) => {
  ...
  const pathnameSuffix = path && path.length > 0
    ...
    : "";
  const targetUrl = `${apiBaseUrl}/api/v1/admin/orders${pathnameSuffix}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: "Unable to reach admin order service",
...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyAdminOrdersRequest(request, context);
}
