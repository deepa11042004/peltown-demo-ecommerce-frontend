...
const proxyAdminCouponsRequest = async (request: NextRequest, context: RouteContext) => {
  ...
  const pathnameSuffix = path && path.length > 0
    ...
    : "";
  const targetUrl = `${apiBaseUrl}/api/v1/admin/coupons${pathnameSuffix}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: "Unable to reach admin coupon service",
...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyAdminCouponsRequest(request, context);
}
